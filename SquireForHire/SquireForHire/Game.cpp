  ...
  ||[ ]||                                            ___________________/
   \===/___________________--------------------------
	...
	while (inGame == true)
	{
		...
		std::cout << INDENT << "What shall you do? Commands: (move + n/s/e/w) (at vendors -> talk OR browse) (use item: use + [item]) (search list for item: search + [item]) (leave)" << std::endl;
		...
		playerInput->ToLower();
		//checks for first character to determin whether it is move/ talk/ browse/ buy/ use/ search
...
void Game::DisplayVendorDescription()
{
	...
	for (int i = 0; i < len; i++)
	{
		...
		if (possibleLocations[i] == searchVal)
		{
			...
			case 7:
				//MAGIC BIN 
				std::cout << "A lone" << BLUE << " bin" << WHITE << " sits in an otherwise empty stall.Nothing odd here.\n" << std::endl;
				return;
				break;
			}
		}
